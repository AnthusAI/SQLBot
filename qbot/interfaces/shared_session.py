...
from qbot.core import QBotAgent, QBotConfig, QueryResult, QueryType
from qbot.conversation_memory import ConversationMemoryManager
from qbot.interfaces.repl.formatting import ResultFormatter
...
@dataclass
class SessionEvent:
    """Event that occurs during a QBot session"""
...
class QBotSession:
    """
    Shared QBot session that can be used by different UI layers.
    ...
    def __init__(self, config: QBotConfig, event_callback: Optional[Callable[[SessionEvent], None]] = None):
        """
        Initialize QBot session
        
        Args:
            config: QBot configuration
            event_callback: Optional callback for session events
        """
        self.config = config
        self.agent = QBotAgent(config)
        ...
        self.session_tree = Tree(f"🤖 QBot Session - Profile: {config.profile}")
    ...
    def execute_query(self, query_text: str) -> QueryResult:
        ...
        try:
            ...
            if query_text.startswith('/'):
                ...
                # Use the working original SQL execution from qbot.repl
                try:
                    from qbot.repl import execute_dbt_sql_rich
                ...
                # Use the working original LLM integration from qbot.repl
                try:
                    from qbot.repl import handle_llm_query, LLM_AVAILABLE, DBT_PROFILE_NAME
                    import qbot.repl as repl_module
...
• exit, quit, q - Exit the session"""
    ...
    def _call_handle_llm_query_safely(self, query_text: str) -> str:
        ...
        """
        from qbot.repl import handle_llm_query
        import qbot.llm_integration as llm_module
...
class QBotSessionFactory:
    """Factory for creating QBot sessions with different configurations"""
    
    @staticmethod
    def create_from_args(args, event_callback: Optional[Callable[[SessionEvent], None]] = None) -> QBotSession:
        """
        Create QBot session from parsed command line arguments
        ...
        Returns:
            Configured QBotSession
        """
        # Create base config from environment
        config = QBotConfig.from_env(profile=getattr(args, 'profile', 'Sakila'))
        ...
        return QBotSession(config, event_callback)
    
    @staticmethod
    def create_for_profile(profile: str, event_callback: Optional[Callable[[SessionEvent], None]] = None) -> QBotSession:
        """
        Create QBot session for a specific profile
        ...
        Returns:
            Configured QBotSession
        """
        config = QBotConfig.from_env(profile=profile)
        return QBotSession(config, event_callback)
