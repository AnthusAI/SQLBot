"""
QBot Theme System - New Architecture

This module provides a unified theming system that leverages Textual's built-in themes
and adds QBot-specific message colors on top. Supports both built-in themes and 
user-defined themes in ~/.qbot/themes/.
"""
...
THEME_ALIASES = {
    "qbot": "tokyo-night",  # Default QBot theme
    "dark": "textual-dark",
    "light": "textual-light",
...
class QBotTheme(BaseModel):
    """QBot theme definition - supports user themes only (built-in themes use Textual directly)"""
    ...
    # QBot-specific message colors (all optional - will use intelligent defaults)
...
def load_user_themes() -> Dict[str, QBotTheme]:
    """Load user themes from ~/.qbot/themes/"""
    from qbot.locations import theme_directory
    ...
    for theme_file in theme_dir.glob("*.yaml"):
        try:
            ...
            theme = QBotTheme(name=theme_name, **theme_data)
...
class QBotThemeManager:
    ...
    def get_css_variables(self) -> Dict[str, str]:
        ...
        if self.is_builtin_theme:
            # For built-in themes, we only provide QBot-specific message colors
...
_theme_manager: Optional[QBotThemeManager] = None

def get_theme_manager() -> QBotThemeManager:
    """Get the global theme manager instance"""
    global _theme_manager
    if _theme_manager is None:
        _theme_manager = QBotThemeManager()
    return _theme_manager

def set_theme_manager(manager: QBotThemeManager) -> None:
