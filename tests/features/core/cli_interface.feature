Feature: Command Line Interface
  As a user of QBot
  I want to use QBot from the command line
  So that I can integrate it into scripts and workflows

  Background:
    Given QBot is installed and available as 'qbot' command

  Scenario: Display help information
    When I run "qbot --help"
  ...
  Scenario: Execute single query from command line
    ...
    When I run 'qbot "How many tables are in the database?"'
  ...
  Scenario: Handle invalid command line arguments
    When I run "qbot --invalid-option"
    Then I should see an error message
    And I should see help information
    And the command should exit with error code

  Scenario: Module execution
    Given QBot is installed
    When I run "python -m qbot.repl --help"
    Then I should see the same help as "qbot --help"
